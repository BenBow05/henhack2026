    ...
    fetch("http://localhost:3001/events")
      ...
      .then((allEvents: Event[]) => {
        const attending = allEvents.filter(event =>
          event.attendees.includes(user?.id ?? "currentUserId")
        );
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="bg-gradient-to-r from-primary to-secondary px-6 py-8 shadow-lg">
        <div className="max-w-6xl mx-auto">
          ...
          <p className="text-lg opacity-90">Your upcoming events and connections</p>
        </div>
      </header>
          ...
          {loading ? (
            ...
            <p className="text-muted-foreground">
              {myEvents.length === 0
                ...
                : `You're attending ${myEvents.length} ${myEvents.length === 1 ? "event" : "events"}`}
            </p>
          )}
        </div>

        {loading ? null : myEvents.length === 0 ? (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-6">
                  ...
                  <div className="p-6 flex flex-col justify-between">
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 mt-6">
                      ...
                      <button
                        onClick={() => router.push(`/Match/${event.id}`)}
